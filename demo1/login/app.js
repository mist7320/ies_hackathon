...
loginForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // Pass the username to the dashboard via the redirect URL
        const username = email.split('@')[0];

        // Redirect to dashboard dev server
        setTimeout(() => {
            window.location.href = `http://localhost:5173/?user=${encodeURIComponent(username)}`;
        }, 1500);
...
signupForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (error) {
            ...
            if (error.message.includes('email rate limit') || error.message.includes('too many requests')) {
                throw new Error('Too many signup attempts. Please wait a few minutes before trying again.');
            }
            throw error;
        }

        showMessage('signup', 'success', 'Account created successfully! Please check your email to verify your account, then sign in.');
